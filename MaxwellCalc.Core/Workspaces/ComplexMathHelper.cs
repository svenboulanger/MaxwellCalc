...
namespace MaxwellCalc.Core.Workspaces
{
    ...
    public static class ComplexMathHelper
    {
        ...
        /// </summary>
        [CalculatorName("i"), CalculatorDescription("Imaginary number.")]
        public static Complex I = Complex.ImaginaryOne;

        /// <summary>
        /// The imaginary number.
        /// </summary>
        [CalculatorName("j"), CalculatorDescription("Imaginary number.")]
        ...
        public static bool Abs(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                result = _invalid;
                diagnostics?.PostDiagnosticMessage(new("Expected single argument for abs()."));
                return false;
            }
        ...
        public static bool Arg(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                result = _invalid;
                diagnostics?.PostDiagnosticMessage(new("Expected single argument for arg()."));
                return false;
            }
        ...
        public static bool Re(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                result = _invalid;
                diagnostics?.PostDiagnosticMessage(new($"Expected single argument for {nameof(Re)}."));
                return false;
            }
        ...
        public static bool Im(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                result = _invalid;
                diagnostics?.PostDiagnosticMessage(new($"Expected single argument for {nameof(Im)}."));
                return false;
            }
        ...
        public static bool Sin(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        public static bool Cos(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleGivenUnitArgument(Unit.UnitRadian, diagnostics, nameof(Sin), out result))
                return false;
        ...
        public static bool Tan(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleGivenUnitArgument(Unit.UnitRadian, diagnostics, nameof(Sin), out result))
                return false;
        ...
        public static bool Sinh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        public static bool Cosh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Tanh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Exp(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Ln(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Log10(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Log2(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (!args.SingleNonUnitArgument(diagnostics, nameof(Sinh), out result))
                return false;
        ...
        public static bool Sqrt(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                // Only one argument allowed
                result = _invalid;
                diagnostics?.PostDiagnosticMessage(new("Expected single argument for sqrt()."));
                return false;
            }
            var arg = args[0];
        ...
        public static bool Asin(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Asin, diagnostics, nameof(Asin), out result);
        ...
        public static bool Acos(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Acos, diagnostics, nameof(Acos), out result);
        ...
        public static bool Atan(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Atan, diagnostics, nameof(Atan), out result);
        ...
        public static bool Asinh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Asinh, diagnostics, nameof(Asinh), out result);
        ...
        public static bool Acosh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Acosh, diagnostics, nameof(Acosh), out result);
        ...
        public static bool Atanh(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Atanh, diagnostics, nameof(Atanh), out result);
        ...
        [MinArg(2), MaxArg(2)]
        public static bool Atan2(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Atan2, diagnostics, nameof(Atan2), out result);
        ...
        public static bool Max(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Max, diagnostics, nameof(Max), out result);
        ...
        public static bool Min(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Min, diagnostics, nameof(Min), out result);
        ...
        [MinArg(1), MaxArg(2)]
        public static bool Round(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
            ...
            else
            {
                ...
                {
                    result = _invalid;
                    diagnostics?.PostDiagnosticMessage(new($"Expected the second argument to be real."));
                    return false;
                }
        ...
        public static bool Factorial(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Factorial, diagnostics, nameof(Factorial), out result);
        ...
        public static bool FactorialLn(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.FactorialLn, diagnostics, nameof(FactorialLn), out result);
        ...
        [MinArg(2), MaxArg(2)]
        public static bool Binomial(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Binomial, diagnostics, nameof(Binomial), out result);
        ...
        [MinArg(2), MaxArg(2)]
        public static bool BinomialLn(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.BinomialLn, diagnostics, nameof(BinomialLn), out result);
        ...
        public static bool Multinomial(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Multinomial, diagnostics, nameof(Multinomial), out result);
        ...
        public static bool Expi(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        public static bool ExpiExp(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        public static bool Exp1(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        public static bool Exp1Exp(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
        {
        ...
        [MinArg(2), MaxArg(2)]
        public static bool Expn(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Expn, diagnostics, "En", out result);
        ...
        public static bool Gamma(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.Gamma, diagnostics, "Gamma", out result);
        ...
        public static bool GammaLn(IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, out Quantity<Complex> result)
            => EvaluateAsReal(args, DoubleMathHelper.GammaLn, diagnostics, "GammaLn", out result);

        private static bool SingleGivenUnitArgument(this IReadOnlyList<Quantity<Complex>> args, Unit unit, IDiagnosticsHandler? diagnostics, string name, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                // Only one argument allowed
        ...
        private static bool SingleNonUnitArgument(this IReadOnlyList<Quantity<Complex>> args, IDiagnosticsHandler? diagnostics, string name, out Quantity<Complex> result)
        {
            if (args.Count != 1)
            {
                // Only one argument allowed
        ...
        private static bool EvaluateAsReal(IReadOnlyList<Quantity<Complex>> args, IWorkspace<double>.BuiltInFunctionDelegate function, IDiagnosticsHandler? diagnostics, string name, out Quantity<Complex> result)
        {
            ...
            for (int i = 0; i < args.Count; i++)
            {
                ...
                {
                    result = _invalid;
                    diagnostics?.PostDiagnosticMessage(new($"Expected real arguments for {name}()."));
                    return false;
                }
                realArgs[i] = new Quantity<double>(args[i].Scalar.Real, args[i].Unit);
            }
            
            if (!function(realArgs, diagnostics, out var realResult))
            {
                result = _invalid;
                return false;
            }
